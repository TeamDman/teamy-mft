...
mod tests {
    ...
    fn postings_candidate_rows_follow_or_of_ands_semantics() {
            ...
            .matching_row_index_candidates(&|rule| {
                Ok(match format!("{rule:?}").as_str() {
                    "ContainsCaseInsensitive(AsciiLower([97, 108, 112, 104, 97]))" => {
                        MatchingRowIndices::RowIndices(vec![0, 1, 3])
                    }
                    "ContainsCaseInsensitive(AsciiLower([98, 101, 116, 97]))" => {
                        MatchingRowIndices::RowIndices(vec![1, 2, 3])
                    }
                    "ContainsCaseInsensitive(AsciiLower([103, 97, 109, 109, 97]))" => {
                        MatchingRowIndices::RowIndices(vec![5])
                    }
        ...
        assert_eq!(candidates, MatchingRowIndices::RowIndices(vec![1, 3, 5]));
    }
