...
15. Completed: common extension-style suffix queries such as `.jar$` now use persisted terminal-segment extension suffix postings in the on-disk search index instead of rebuilding an in-memory suffix map per query parse.
16. Completed: the search-index format was intentionally bumped to version 5 for persisted extension suffix postings, then to version 6 for persisted trigram contains postings, and then to version 7 so `$` semantics only match terminal path segments in persisted suffix indexes.
